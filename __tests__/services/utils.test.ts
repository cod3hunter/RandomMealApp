import {normalizeIngredients} from '../../src/services/utils';
...
describe('utils', () => {
  describe('normalizeIngredients', () => {
    test('should return array of string with ingredients and measures', () => {
      const normalizedData = normalizeIngredients(mealData);
      expect(normalizedData.length).toBe(14);
      expect(normalizedData).toContain(
        `${mealData.strIngredient10} - ${mealData.strMeasure10}`,
      );
    });
  });
});
